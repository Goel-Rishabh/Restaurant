...
export class ShowDishes extends Component {
    ...
    postDishNavigate=(id,name)=>{
        this.props.navigation.navigate("PostDish",{id:id,name:name,updateScreen:this.getData})
    }
    ...
    addCategory=async(name,closeRBSheet)=>{
        try{
            const url = urls.CATEGORY
            const token = await AsyncStorage.getItem(storageKeys.TOKEN)
            const headers = {'Content-Type': 'application/json', 'Accept': 'application/json','Authorization':`bearer ${token}`}
            let resp = await postRequest(url,headers,JSON.stringify({name:name}))
            resp = await resp.json()
            closeRBSheet()
            this.setState({Snackbar:true,snackbarMsg:resp.name+' Added Successfully',newCategory:''})
            this.getData()
        }catch(err){
            this.setState({Snackbar:true,snackbarMsg:' Some Error Occured'})
        }
    }

    deleteCategory=async(dish)=>{
        if(dish.dishes.length>0){
            this.setState({Snackbar:true,snackbarMsg:'Seems This Category Have Some Dishes',confirmDialog:false})
            return
        }
        try{
            const url = urls.CATEGORY+"?id="+dish._id
            const token = await AsyncStorage.getItem(storageKeys.TOKEN)
            const headers = {'Content-Type': 'application/json', 'Accept': 'application/json','Authorization':`bearer ${token}`}
            let resp = await deleteRequest(url,headers)
            resp = resp.json()
            this.setState({Snackbar:true,snackbarMsg:'Category Deleted Successfully',confirmDialog:false})
            this.getData()
        }catch(err){
            this.setState({Snackbar:true,snackbarMsg:'Some Error Occured',confirmDialog:false})
        }
        ...
        <View style={{backgroundColor:colors.TILE,paddingVertical:STATUSBAR_HEIGHT,paddingHorizontal:10,borderBottomLeftRadius:10,borderBottomRightRadius:10}}>
            ...
            <Animated.View style={{width:this.state.width,height:this.state.height,alignSelf:'flex-end'}}>
                <View style={{flexDirection:'row',justifyContent:'flex-end'}}>
                    <View style={{flex:2}}>
                        <TouchableOpacity onPress={()=>{props.openTextInput()}} style={{margin:5}}>
                            <Icon size={25} style={{padding:3}} name={"add-circle-outline"} color={colors.WHITE} />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={()=>{this.setState({confirmDialog:true})}} style={{margin:5}}>
                            <FeatherIcon size={25} style={{padding:3}} name={"minus-circle"} color={colors.WHITE} />
                        </TouchableOpacity>                        
                    </View>
                    <View style={{backgroundColor:'white',borderRadius:5,flex:8}}>
                        <ScrollView>
                            {   
                                this.state.categories.length==0?
                                <Text style={{textAlign:'center',color:colors.TEXT_SECONDATY,padding:10}}>Add Some Categories To See Here</Text>
                                :
                                this.state.categories.map((category,index)=>(
                                    <TouchableOpacity 
                                        style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',padding:8}}
    ...
    DishesTile=(props)=>(
        <View style={{borderRadius:5,flexDirection:'column',backgroundColor:colors.OVERLAY_BACKGROUND,paddingHorizontal:10,marginTop:5}}>
            ...
            <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',}}>
                ...
                <TouchableOpacity style={{padding:10,alignSelf:'flex-start'}} onPress={()=>{this.deleteDish(props.categoryId,props.dishId)}} >
                    <Icon size={25} color={'#ff0000'} name="delete" />
                </TouchableOpacity>
            </View>
            {
                props.description &&
    ...
    deleteDish=async(category,dish)=>{
        this.setState({loading:true})
        try{
            const token = await AsyncStorage.getItem(storageKeys.TOKEN)
            const headers={'Authorization':'bearer '+token,'Accept': 'application/json','Content-Type': 'application/json'}
            const url = urls.DELETE_DISH+`?category=${category}&dish=${dish}`
            let resp = await deleteRequest(url,headers)
            this.getData()
            this.setState({snackbarMsg:'deleted',Snackbar:true,loading:false})
        }catch(err){
            this.setState({snackbarMsg:'Some Error Occured',Snackbar:true,loading:false})
        }
    ...
    render(){
        return(
            <View style={{flex:1,backgroundColor:colors.BACKGROUND}}>
                {/* <TabBarHeader headerText={"Add Dishes"} /> */}
                                ...
                                return(
                                    <View style={{backgroundColor:colors.OVERLAY_BACKGROUND,marginTop:5}}>
                                        <View style={{flexDirection:'row',marginLeft:20,alignItems:'center',justifyContent:"space-between",marginTop:30}}>
                                            ...
                                            <TouchableOpacity onPress={()=>{this.postDishNavigate(_id,name)}} style={{marginRight:20}}>
                                                <Icon size={25} color={colors.TILE} name={"add"} />
                                            </TouchableOpacity>
                                        </View>
                                        <View>
                                            {
